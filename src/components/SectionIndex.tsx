...
export default function SectionIndex() {
  ...
  return (
    <section className="w-full bg-[#0B0A10] border-y border-white/[0.08] py-8 overflow-hidden font-sans relative">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="flex items-center justify-between mb-6">
          <span className="font-mono text-[9px] text-[#8C4BFF] tracking-[0.2em] font-bold">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-0 lg:divide-x lg:divide-white/[0.08] border-t border-white/[0.08] pt-6">
          {tracks.map((track, i) => (
            ...
            >
              ...
              <div className="absolute top-0 left-0 right-0 h-[2px] bg-transparent group-hover:bg-[#8C4BFF]/40 transition-colors" />

              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[10px] text-neutral-500 font-bold">TRACK // {track.num}</span>
                  <span className="font-mono text-[9px] text-[#8C4BFF] font-semibold">{track.frame}</span>
                </div>
                <h4 className="text-sm font-black uppercase text-white tracking-wide group-hover:text-[#8C4BFF] transition-colors mt-2">
                  {track.title}
                </h4>
                <p className="text-[11px] text-[#B9B2C2] font-light leading-snug">
                  {track.desc}
                </p>
              </div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-[#8C4BFF] opacity-0 group-hover:opacity-100 transition-opacity">
                  GO TO LAYER →
                </span>
              </div>
            </div>
