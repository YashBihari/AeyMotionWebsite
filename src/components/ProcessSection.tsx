...
import { Search, Target, FileText, Film, Layers, LineChart } from 'lucide-react';
import AeymotionBackground from './AeymotionBackground';
import { PROCESS_SECTION } from '../data/content';

export default function ProcessSection() {
  const stepIcons = [Search, Target, FileText, Film, Layers, LineChart];
    ...
    <section id="process" className="py-10 md:py-14 bg-transparent font-sans relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="mb-6 md:mb-8">
          ...
          <p className="text-neutral-600 font-normal text-sm md:text-base max-w-2xl mt-2.5 leading-relaxed">
            {PROCESS_SECTION.subtitle}
          </p>
        </div>

        {/* 6-Step Process Timeline Cards (2x3 Grid) */}
          ...
          {PROCESS_SECTION.steps.map((step, idx) => {
            ...
            return (
              <motion.div
                ...
                className="p-6 sm:p-7 bg-white/85 backdrop-blur-md border border-purple-100/90 rounded-2xl shadow-xs hover:border-[#8C4BFF]/50 hover:shadow-xl hover:shadow-[#8C4BFF]/10 transition-shadow duration-300 flex flex-col justify-between relative group cursor-pointer select-none"
              >
                <div>
                  <div className="flex items-center justify-between mb-5">
                    <span className="font-mono text-[10.5px] font-bold text-[#8C4BFF] uppercase tracking-widest bg-[#8C4BFF]/10 px-2.5 py-0.5 rounded-md">
                      STAGE // {step.number}
                    </span>
                ...
                <div className="mt-6 pt-3 border-t border-neutral-100 flex items-center justify-between text-[10px] font-mono text-neutral-400">
